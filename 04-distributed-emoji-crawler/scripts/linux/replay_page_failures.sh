...
exec 9>"${RUN_DIR}/replay-page-${SPIDER_NAME}.lock"
if ! flock -n 9; then
    echo "page replay already running for ${SPIDER_NAME}; skipped"
    exit 0
fi

cd "${PROJECT_ROOT}"
"${VENV_PYTHON}" "${PROJECT_ROOT}/failure_tasks.py" \
    replay page --spider "${SPIDER_NAME}" --all --limit "${REPLAY_LIMIT}"
